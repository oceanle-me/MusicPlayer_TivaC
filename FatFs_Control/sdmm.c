...
)
{
    ...
    do {
        ...
        SSIDataPut(SSI0_BASE, d); //send data

        SSIDataGet(SSI0_BASE, &rx_temp); //flush data read during the write
...
)
{
    ...
    do {
        SSIDataPut(SSI0_BASE, 0xFF); //send 0xFF
        SSIDataGet(SSI0_BASE, &rx_temp);
        *buff++ = rx_temp;          /* Store a received byte */
    } while (--bc);
}
